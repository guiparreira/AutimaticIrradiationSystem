...
namespace autoDif
{
    public partial class MainForm : Form
    {
        ...
        //Média temperatuera
        bool tempAvgMode = false;
        int maxTempSamples = 1000;
        int sampleCounter = 0;
        ...
        public static void saveConfig()
        {
            ...
            Machine.doseRateCalibDist.ToString(), Machine.doseRateCalibVolt.ToString(), Machine.doseRateCoefA.ToString(), Machine.doseRateCoefB.ToString()};
       ...
       async Task getAvgImg()
        ...
        async void manageDistMeasurement()
        {
            ...
            switch(distMeasurementModeState)
            {
                ...
                case 1:
                    await getAvgImg();
                    if(DEBUG_MODE) trackingSample.Save(Path.Combine(filePath, "trackSample.png"));
...
/*#if DEBUG
        ...
        private void videoSourcePlayer_Paint(object sender, PaintEventArgs e)
        {
            ...
            if(enableRuler)
            {
                ...
                if(rulerMode)
                {
                    ...
                    //,3e.Graphics.DrawLine(penRedLine, Math.Abs(rulerP1.X - rulerP2.X), )
        ...
        void updateDisplayInfo()
        {
            ...
            if (tempAvgMode) lblTemp.ForeColor = Color.Red;
            else lblTemp.ForeColor = Color.Black;

            if (Machine.temperature > 100) lblTemp.Text = "Temp. Error";
            ...
            if(Machine.autoPos)
            {
                if (Machine.sampleInPos)
                {
                    ...
                    rbAutoManual.Enabled = false;
                    rbAutoPos.Enabled = false;
                }
                else
                {
                    btnGoToRadPos.Text = "Move to target";
                    rbAutoManual.Enabled = true;
                    rbAutoPos.Enabled = true;
                }
            }
            else
            {
        ...
        void updateInfo(char cmd, string val)
        {
            switch (cmd)
            {
                ...
                case 't':
                    ...
                    if(tempAvgMode)
                    {
                        tempSamples[sampleCounter] = Machine.temperature;
                      
                        string data = sampleCounter * 5 + "\t" + tempSamples[sampleCounter] + "\n";
                        File.AppendAllText(Path.Combine(filePath, "tempLog.txt"),  data);
...
//==================================================================================================================
        ...
        {
            //Ajusta pequeno desvio - X

            double step = Machine.targetOffset().x;
            if(Machine.manualPosMode && Math.Abs(step) <= minDistAutoAdj)
            sendStepCmd(-step, 0, 0);
        }
        private void lblOffsetY_Click(object sender, EventArgs e)
        {
            //Y
            double step = Machine.targetOffset().y;
            if (Machine.manualPosMode && Math.Abs(step) <= minDistAutoAdj)
                sendStepCmd(0, -step, 0);
        }

        private void label13_Click(object sender, EventArgs e)
        {
            //z
            double step = Machine.targetOffset().z;
            if (Machine.manualPosMode && Math.Abs(step) <= minDistAutoAdj)
            sendStepCmd(0, 0, -step);
        }
        ...
        //Abre painel calculo taxa de dose
        private void doseRateCalculatorToolStripMenuItem_Click(object sender, EventArgs e)
        {
            DoseRateCalculator s = new DoseRateCalculator();
            s.ShowDialog();
        }
        ...
        private void lblTemp_DoubleClick(object sender, EventArgs e)
        {
            if (!tempAvgMode)
            {
                tempSamples = new double[maxTempSamples];
                sampleCounter = 0;
                tempAvgMode = true;
           
                File.WriteAllText(Path.Combine(filePath, "tempLog.txt"), "Tempo (s)\tTemp(C)\n");
                MessageBox.Show("Iniciando coleta de dados - temperatura média");
            ...
            else
            {
                tempAvgMode = false;
                double avg = 0;
                string[] data = new string[sampleCounter];

                for (int i = 0; i < sampleCounter; i++)
                {
                    avg += tempSamples[i];
                    data[i] = i * 5 + "\t" + tempSamples[i];
                }
                avg /= sampleCounter;

                int tempo = sampleCounter * 5;

                //File.WriteAllLines(Path.Combine(filePath, "tempLog.txt"), data);

                MessageBox.Show("Temp. Média: " + avg + "ºC \n Tempo decorrido: " + tempo + " segundos");
